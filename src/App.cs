...
public class App 
{
    //For now the Run() method only creates the account.
    public void Run()
    {
        CreateAccount();
    }
    ...
    /*
    Method returns the created account, I have added this to further improve the app with a DataBase. Saving each instance will make it better to put in the DB.
    */
    ...
    {
        return !string.IsNullOrEmpty(str) && !str.Any(char.IsDigit) && !str.Any(char.IsWhiteSpace) && !str.Any(char.IsAsciiLetter);
    }
}